...
def main():
    ...
    temp = MOX_classifier.fit(train_X, train_Y)
    y_score = temp.decision_function(valid_X)
    ...
    @deterministic
    def f1(CO_classes, valid_Y, y_score):
        fpr=dict()
        tpr=dict()
        roc_auc=dict()
    ...
    fpr, tpr, roc_auc = f1(CO_classes, valid_Y, y_score)
    ...
    for one_class, one_color in zip(range(CO_classes), ROC_Color):
        temp = 'ROC fold {0} (AUC = {1:0.2f})'
        plt.plot(fpr[one_class], tpr[one_class], color=one_color, lw=1,label=temp.format(one_class, roc_auc[one_class]))
    plt.title('ROC')
    ...
    ######plt.show()
    plt.savefig("out/OneVsRestClassifier_with_ROC.png")
